...
import Migrating from "main/mixins/migrating.js";
import mime from "mime";
import { noop, typeBySuffix } from "./unit.js";
        ...
        fileList: {
            ...
            handler(fileList) {
                this.uploadFiles = fileList.map(item => {
                    item.type = item.type || typeBySuffix(item[this.defaultOpt.name]);
    ...
    methods: {
        handleStart(rawFile) {
            ...
            let file = {
                ...
                name: rawFile.name,
                size: rawFile.size,
                type: mime.getExtension(rawFile.type),
                percentage: 0,
                uid: rawFile.uid,
                raw: rawFile
            };
        ...
        handleRemove(file, raw, sizeBool) {
            if (raw) {
            ...
            let doRemove = () => {
                ...
                fileList.splice(fileList.indexOf(file), 1);
                this.onRemove(file, fileList, sizeBool);
            };

            if (!this.beforeRemove) {
                doRemove();
            } else if (typeof this.beforeRemove === "function") {
                const before = this.beforeRemove(file, this.uploadFiles, sizeBool);
