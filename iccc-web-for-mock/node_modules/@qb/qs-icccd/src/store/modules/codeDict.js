import { getCodeDict } from '@icccdPath/api/common';
import { deepClone } from '@icccdPath/utils/tool';
...
const codeDict = {
  ...
  actions: {
     ...
     */
    getBmList({ state, commit }, codeId) {
      return new Promise((resolve, reject) => {
        // 加载状态
        const loadStatus = state.loaded[codeId];
        // 1 表示加载完成，可以直接返回
        if (loadStatus === 1) {
          resolve(deepClone(state.dictList[codeId]));
          return;
        }
        // 添加到订阅事件
    ...
    async getBm({ dispatch }, codeId) {
      const obj = {};
      const list = await dispatch('getBmList', codeId);
      list.forEach(({ label, value }) => {
        obj[value] = label;
      });
      return obj;
    },
    async getBmMc({ dispatch }, { codeId, key }) {
      // 先从这里面取，可能有优化空间
      const list = await dispatch('getBmList', codeId);
