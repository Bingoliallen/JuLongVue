 ...
 */
function gulpScssMerge(fileName, basePath) {
  /**
  ...
  function getFileContent(filePath) {
    ...
    let result = '';
    if (allFileKeys[filePath]) {
    } else {
      allFileKeys[filePath] = 1;
  ...
  function getImportPath(scssImport, path) {
    ...
    path = path.trim();
    path = Path.parse(path).ext === '.scss' ? Path.parse(path).dir : path;
    ...
    return Path.parse(scssImport).ext === '.scss'
      ? Path.join(path, Path.dirname(scssImport.trim()), Path.basename(scssImport))
      : Path.join(path, Path.dirname(scssImport.trim()), Path.basename(scssImport) + '.scss');
  }
