    ...
    <div class="monitor-list-detail">
      ...
      >
        <template v-slot:needfollow>
          <el-button v-if="showAgree" type="primary" @click="bkspAgree(1)">同意</el-button>
          <el-button v-if="showReturn" @click="bkspAgree(0)">退回</el-button>
        </template>
        <template v-slot:title="{ form }">
          <span class="main-label__slot">
            <span
              >{{ form.sqmc }}
              <span v-if="isTk"
                >（共<a class="font-yellow">{{ form.bkdx ? form.bkdx : '0' }}</a
                >个）</span
              >
            </span>
          </span>
            ...
            <el-row>
              ...
              <el-col :xs="14" :sm="14" :md="14" :lg="14" :xl="12">
                <el-form-item label="申请类别：">
                  <span class="font-yellow" v-code="{ code: 'BM_BKSQ_BKLX', value: form.bklx }"></span>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
      ...
      <div class="detail-list-container">
        ...
        <div class="detail-list-container__center">
          <keep-alive>
            ...
            <control-target class="control-target" :id="tempId" v-if="tabItemShow === 'tkdx'"></control-target>
...
export default {
  ...
  computed: {
    // 是否是停控信息
    isTk() {
      return this.info.bklx === '31' || this.info.bklx === '32';
    },
    // 显示同意
    showAgree() {
      return this.shortSecurityOrg === this.info.dqspdw || this.securityOrg === this.info.dqspdw;
    },
    // 显示退回
    showReturn() {
      return (
        this.info.sqdzt !== '21' &&
        (this.shortSecurityOrg === this.info.dqspdw || this.securityOrg === this.info.dqspdw)
      );
    }
  },
  // 方法
